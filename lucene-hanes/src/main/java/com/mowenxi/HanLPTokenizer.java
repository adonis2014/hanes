...
public class HanLPTokenizer extends Tokenizer {
    ...
    /**
     * Creates a new HanLPTokenizer, supplying the AttributeFactory
     *
     * @param segment              HanLP中的某个分词器
     * @param filter               停用词
     * @param enablePorterStemming 英文原型转换
     */
    public HanLPTokenizer(Segment segment, Set<String> filter, boolean enablePorterStemming, AttributeFactory factory) {
        super(factory);
        this.segment = new SegmentWrapper(new BufferedReader(input), segment);
        this.filter = filter;
        this.enablePorterStemming = enablePorterStemming;
